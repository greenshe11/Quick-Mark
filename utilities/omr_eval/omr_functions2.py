...
import numpy as np
import sys
from utilities.ocr.function import Predict
char_predictor = Predict
on_android = False
try:
    print("RUN")
                ...
                if h != 0:  # Avoid division by zero
                    ...
                    if (ratio >= 0.6 and ratio <= 1.4):
                        ...
                        for rect in rectangles:
                            ...
                            if distance((rectx_center,recty_center),(x_center,y_center)) < 5:
...
def get_bubbles(BubbleGetter_obj, BoxGetter_obj, CaptureSheet_obj, boxes_num, redo=False, use_rect=False,param_value=11):
    ...
    try:
                ...
                if h != 0:  # Avoid division by zero
                    ...
                    if (ratio >= 0.7 and ratio <= 1.3):
                        ...
                        if not_inside:
                            ...
                            print('size',w,h)
                            rectangles.append((x,y,w,h))
                            #cv2.imwrite('bubble_template.png',adaptive_thresh[y:y+h, x:x+w])
                            #cv2.rectangle(image, (x, y), (x + w, y + h), (255,0,0), 5)
        ...
        name = str(redo)+str(use_rect)+str(CaptureSheet_obj)[1:-1]+str(datetime.now().strftime('%Y-%m-%d-%H-%M-%S')) + '.png'
        cv2.imwrite(storage+name,adaptive_thresh)
        #print("SAVED IN ",str(CaptureSheet_obj.storage)+'/'+name)         
        #
        db.plot(image, 'perspective transformed')
        #db.p('qualified contoures ---',len(rectangles))
...
def get_idtf_score(CaptureSheet_obj, BubbleGetter_obj):
    user_answers = BubbleGetter_obj.answers # is a list with elements containing list of character
    print("ANSWERS", user_answers)
    total_poss_answers = []
    ground_truth = CaptureSheet_obj.idq.correct # is a string
    #correct input: ['ABCD','ABCD']
    #answer_input: [[['A'],['C]],[['D'],['E']]]
    if not on_android:
        ground_truth = ['KEVIN JOSEPH','EUGENE DAVE','ROAN SCHUYLER']
    for correct, answer_temp in zip(ground_truth, user_answers):
        correctness = []
        for corr_let, ans_arr in zip(correct,answer_temp):
            #if correct == answer:
            #if correct == answer:=
            answer = tolerate_equiv(ans_arr)
            total_poss_answers.append(answer)
            if corr_let in ['',' ']:
                pass
            elif corr_let in answer:
                print(corr_let, answer,'TRUE')
                
                correctness.append(1)
            
            else:
                print(corr_let, answer,'FALSE')
                correctness.append(0)
        print(correctness)
        if sum(correctness) >= len(correctness) - 1 and len(correctness) > 0:
        #if all(correctness) and len(correctness) > 0:
            #print(True)
            #print((answer_temp))
            #answer_repr = ''.join(['x' if x[0] not in [' ',''] else ' ' for x in answer_temp]).rstrip()
            #answer_repr = answer_repr.split(' ')
            #correct_repr = correct.split(' ')
            #if all([len(x)==len(y) for x,y in zip(answer_repr, correct_repr)]):
            if True:
            ...
            print(False)
            BubbleGetter_obj.eval_array.append(0)
    print(total_poss_answers)
    BubbleGetter_obj.final_score = sum(BubbleGetter_obj.eval_array)
    print(sum(BubbleGetter_obj.eval_array))
    print("EVAL",BubbleGetter_obj.eval_array)
    reset_eval_array = CaptureSheet_obj.check_session._reset_order(BubbleGetter_obj.eval_array,10)
    # the same, the important thing is only whether its considered correct or wrong
    reset_user_answer = CaptureSheet_obj.check_session._reset_order(BubbleGetter_obj.eval_array,10)
    print(BubbleGetter_obj.eval_array)
    print(reset_eval_array)
    CaptureSheet_obj.check_session.idtf_answer = reset_user_answer
    CaptureSheet_obj.check_session.idtf_eval_array = reset_eval_array

def tolerate_equiv(letters):
    """_summary_

    Args:
        letters (array): _description_
    ...
    for i in range(len(equivs)):
        for let in letters:
            if let in equivs[i]:
                return_value = return_value + equivs[i]
            else:
                return_value = return_value + [let]
...
def get_scores(BubbleGetter_obj, BoxGetter_obj, CaptureSheet_obj, boxes_num):
    if BubbleGetter_obj.test_type == 'IDENTIFICATION':
        get_idtf_score(CaptureSheet_obj, BubbleGetter_obj)
        return 
    try:
        ...
        for number in choices_by_num.keys():
            ...
            for rect in choices_by_num[number]:
                ...
                if rate2 < 90:
                    rect_output = rate
                    filled.append(rate)
                    #cv2.imwrite('filled_template.png',cropped_image)
                else:
...
def evaluate_identification(CaptureSheet_obj, bubbles):
    ...
    print("STARTING EVALUATING IDENTIIFCATION")
    cropped_image = CaptureSheet_obj.boxes.crops[0]
    names = []
    print(bubbles)
    bubbles_xywh = []
    image =  cv2.adaptiveThreshold(cropped_image, 255, cv2.ADAPTIVE_THRESH_MEAN_C, cv2.THRESH_BINARY, 17, 2)
    print(len(bubbles))
    iter = 0
    for x in bubbles:
        temp_name = []
        row_x = np.array(x)
        axis = 1 if on_android else 0
        sorted_indices = np.argsort(row_x[:, axis])
        sorted_data = row_x[sorted_indices]
        temp_row_bubbles = [] 
        for rect in sorted_data:
            #x,y,w,h = rect
            x,y,w,h = rect
            temp_row_bubbles.append(rect)
            new_w = int(w * 0.7)
            new_h = int(h * 0.7)
            ...
            charbox = image[new_y:new_y+new_h, new_x:new_x+new_w]
            #print()
            #cv2.imwrite(storage+f'{iter}.png',charbox)
            prediction,probability = char_predictor(charbox,iter)
            iter+=1
            #probability = probability[0]
            cropped_image = charbox
            rate2 = np.sum(cropped_image[cropped_image >= np.mean(cropped_image)])
            ##*print(rate2)
            cropped_image = cv2.adaptiveThreshold(cropped_image, 255, cv2.ADAPTIVE_THRESH_MEAN_C, cv2.THRESH_BINARY, cropped_image.shape[0] - 1 - cropped_image.shape[0]%2, 2)
            rate2 = np.sum(255-cropped_image)/np.sum(np.ones(cropped_image.shape))
            ...
            if rate2 > 15:
                if True:
                    temp_name.append(list(probability.keys())[-3:]) # get top 6
                else:
                    temp_name.append([str(prediction)])
            else:
                temp_name.append([" "])
            
            #cv2.rectangle(cropped_image, (x, y), (x + w, y + h), (0,255,0), 20)
            #db.plot(charbox)
            
        bubbles_xywh.append(temp_row_bubbles)    
        #print("NAME", temp_name)
        CaptureSheet_obj.bubbles[0].answers = temp_name
        names.append(temp_name)

        
    CaptureSheet_obj.bubbles[0].answers = names
...
def get_choices_by_num(BubbleGetter_obj, BoxGetter_obj, CaptureSheet_obj, box_index, **kwargs):
    ...
    try:
        ...
        print('IDENTIIFICATION EVALUATION DONE!')
