...
from utilities.omr_eval.omr_functions2 import Debugger
import cv2
...
class CameraWidget(BoxLayout):
    def __init__(self, mcq_correct = None, tf_correct= None, idtf_correct = None, primary_storage=None, check_session=None,checkscreen=None,**kwargs):
        ...
        self.left_padding = BoxLayout(size_hint=(0.1,1))
        self.right_padding = BoxLayout(size_hint=(0.1,1))
        ...
        self.frame_image = Image(source='assets/loading.jpg',size_hint=(0.8,1), pos_hint={'center_x': 0.5, 'center_y': 0.4},allow_stretch=True, keep_ratio=False)
        self.add_widget(self.left_padding)
        #self.add_widget(self.frame_image)
        self.add_widget(self.right_padding)
                        ...
                        if self.eucdist(previous_cs_point, (x,y)) < 40:
                            ...
                            if len(self.cs_points) == 5:
                                ...
                                #for obj in self.cs_objs:
                                    ...
                                    def type_id(obj):
                                        try:
                                            return obj.bubbles[0].count == len(self.idtf_correct)*15 if len(self.idtf_correct) != 0 else False
                                        except Exception as e:
                                            print("EXCEPTION",e)
                                            return False
                                    ...
                                    if any([func(obj) for func in [type_mc, type_tf, type_id]]):
                                        ...
                                        try:
                                            ...
                                            if test_type == 'MULTIPLE CHOICE':
                                                self.check_session.get_mc_score()
                                            elif test_type == 'TRUE OR FALSE':
                                                self.check_session.get_tf_score()
                                            else:
                                                self.check_session.get_idtf_score()
                                            ...
                                            self.check_screen.update(test_type,ignore_img=True)
                                            self.label.text = '\n'.join([str(counting), 'SCORE:'+str(score),'TYPE:'+str(test_type)])
